...
| `@fadinglight/dsh-tool-harmonyos-uitest` | `devecocli` | `ctx.tools`、`ctx.subprocess`、`ctx.systemPrompt` | `tool/call`、`tool/result` | - | 已安装的 deveco-cli skill 拥有命令指导；工具仅以普通 argv 接受 skill 记录的顶层命令族。 |
...
使用普通 argv 向量运行受 allow-list 限制的 DevEco CLI 命令。
...
来源：[`packages/harmony/tool-harmonyos-uitest/src/index.ts`](../packages/harmony/tool-harmonyos-uitest/src/index.ts)

已安装的 deveco-cli skill 拥有命令指导；工具仅以普通 argv 接受 skill 记录的顶层命令族。
