...
| `@deepseek-ai/dsh-tool-harmonyos-uitest` | `devecocli` | `ctx.tools`、`ctx.subprocess`、`ctx.systemPrompt` | `tool/call`、`tool/result` | - | 已安装的 deveco-cli skill 拥有命令指导；工具仅以普通 argv 接受 skill 记录的顶层命令族。 |
