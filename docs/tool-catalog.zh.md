...
| `@deepseek-ai/dsh-tool-harmonyos-uitest` | `devecocli` | `ctx.tools`、`ctx.subprocess`、`ctx.systemPrompt` | `tool/call`、`tool/result` | - | 已安装的 deveco-cli skill 拥有命令指导；工具仅以普通 argv 接受固定的基础命令族。 |
...
已安装的 deveco-cli skill 拥有命令指导；工具仅以普通 argv 接受固定的基础命令族。
