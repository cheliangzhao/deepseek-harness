...
| `@fadinglight/dsh-tool-harmonyos-uitest` | `devecocli` | `ctx.tools`, `ctx.subprocess`, `ctx.systemPrompt` | `tool/call`, `tool/result` | - | The installed deveco-cli skill owns command guidance; the tool accepts only the skill-documented top-level command families as plain argv. |
