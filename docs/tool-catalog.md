...
| `@deepseek-ai/dsh-tool-harmonyos-uitest` | `devecocli` | `ctx.tools`, `ctx.subprocess`, `ctx.systemPrompt` | `tool/call`, `tool/result` | - | The installed deveco-cli skill owns command guidance; the tool accepts only the skill-documented top-level command families as plain argv. |
...
Run an allow-listed DevEco CLI command with a plain argv vector.
...
Source: [`packages/harmony/tool-harmonyos-uitest/src/index.ts`](../packages/harmony/tool-harmonyos-uitest/src/index.ts)

The installed deveco-cli skill owns command guidance; the tool accepts only the skill-documented top-level command families as plain argv.
