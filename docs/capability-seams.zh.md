...
| `ctx.deviceAutomation` | `seam` | `device-automation-runtime` | `device-automation-harmonyos` | `ui-device-automation` | - | 平台 Provider 负责设备命令；运行时负责受信浏览器 RPC 和工作区文件访问；浏览器 Consumer 负责文件与设备界面。 |
