...
Browser Consumer for the device automation capability. Automation sessions gain one header shortcut that opens a compact details workspace with only two surfaces: **Files** and **Device**.
...
Device requests complete screenshots through the same trusted RPC channel, keeps the previous frame until its replacement decodes, and waits for the Host-advertised interval after each completed request. Polling stops while the browser document is hidden or the Files tab is selected. Failed captures keep retrying so a newly connected authorized device appears without reopening the panel. Clicking decoded image content sends relative coordinates; letterboxing and loading states do not issue taps.
