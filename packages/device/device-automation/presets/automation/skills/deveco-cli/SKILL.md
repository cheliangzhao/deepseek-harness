...
name: deveco-cli
description: >-
  **MUST** use 'devecocli' for HarmonyOS apps (scaffold, build, run, debug, devices, emulators, logs, docs, 鸿蒙开发文档, skills). **REQUIRED** and **MANDATORY** if workspace has build-profile.json5 or oh-package.json5, or user mentions HarmonyOS, 鸿蒙, DevEco, ArkTS, ArkUI, ui automation, UI 自动化测试, uitest, docs, 鸿蒙文档, 知识, 搜索, or skills.
---
...
Available commands: `build`, `run`, `update`, `device`, `emulator`, `skills`, `log`, `create`, `init`, `serve`, `docs`, `ui`, `auth`, `check`, `signature`.
...
### `devecocli ui`
Inspect and interact with the UI on a connected device (wraps the on-device UITest framework; same capabilities as `hdc shell uitest`). Most subcommands take `--device <name|serial>` (Req on multi-device hosts).
- `layout`: Inspect the on-screen node tree. Opts: `--id <id>`, `--window <windowId>` / `--all-windows` (mutually exclusive), `--depth <n>` (0=unlimited), `--format default|json`, `--mode full|simplified` (default simplified).
- `window list`: List windows. Opts: `--format default|json`, `--all` (include system windows).
- `screenshot`: Capture screen. Req `--path <dir-or-png>` (writable); opt `--display <displayId>`.
- `click|doubleclick|longclick [x y]`: Tap by coordinates, or by node with `--id <id>` (+`--window <windowId>`; auto-resolves node center).
- `swipe|fling|drag <x1> <y1> <x2> <y2>`: Pointer gestures. Opt `--speed <px-per-s>` (swipe).
- `dircfling <direction>`: Directional fling (0=left, 1=right, 2=up, 3=down).
- `text <text> [x y]`: Input text at coordinates, `--id <id>` node center, or the focused field when neither is given.
Note: `wukong` (stability testing) is NOT exposed here — it is an on-device tool, run via `hdc shell wukong ...`.

### `devecocli check`
Run DevEco project checks.
- `compat [files...]`: Source compatibility vs a target SDK version. Req `--source-version` / `--target-version` (run `compat versions` to list; on zsh quote values — they contain parentheses). Project-level scan by default; positional `files...` = file-level; `--modules <m...>` = module-level (mutually exclusive with files). Opts: `--format default|json` (console), `csv|json|default` with `--output-path <dir>` (csv requires it).
- `lint [path]`: DevEco Code Linter for TS/ArkTS. Opts: `--fix`, `--incremental` (uncommitted files only), `--config-path <path>`, `--product <name>`, `--format default|json`, `--output-path <path>`, `--limit <n>` (terminal issues when no `--output-path`).
...
### `devecocli auth`
Huawei Developer account commands: `login` / `logout` / `status` / `team`.

### `devecocli signature`
`generate`: Automatically generate signing materials and write them into the project configuration. Opts: `--force`, `--team-id <id>`, `--product <name>` (default `default`).
...
- **Inspect and drive device UI**:
  `devecocli ui layout --format json` -> `devecocli ui screenshot --path ./shot.png` -> `devecocli ui click --id <node-id>`
...
- **`devecocli ui` fails while AAMS is occupied**: uitest/wukong/hypium share the device accessibility service (AAMS) — one consumer at a time. Close the other tool or restart the device.
- **This skill file is shared across agents**: it lives at `~/.agents/skills/deveco-cli/`; agent config dirs symlink to it. `devecocli update` may overwrite local edits from its bundled template — re-apply after updating.
