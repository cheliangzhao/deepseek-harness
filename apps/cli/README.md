...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-headless`, `@fadinglight/dsh-device-automation`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
