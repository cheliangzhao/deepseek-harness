...
// Boots the REAL web composition — the dsh-base, dsh-web-app, and device automation bundle
...
/** The shipped composition under test: the Web profile's Bundle patches over the empty profile root. */
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const automationPatches = loadOverlayPatches('web e2e scaffold', DEVICE_AUTOMATION_PATCH_PATH)
  const extraOverlayPatches = options.extraOverlayPath === undefined
    ? []
    : loadOverlayPatches('web e2e scaffold', options.extraOverlayPath)
  const composedRows = composeEntries([basePatches, surfacePatches, automationPatches, extraOverlayPatches])
