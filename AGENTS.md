...
apps/        Deployable applications: `apps/cli` (the `dsh` launcher) and `apps/web` (browser frontend)
...
Package groups: [packages/README.md](packages/README.md) is the authoritative group map. The web GUI splits across `packages/host/` (server) and `packages/client/` (browser); `packages/harmony/` runs HarmonyOS device automation through the DevEco CLI.
...
pnpm vitest run <path> [-t <name>]  # focused run: file, dir, or name filter (never full suite for one change)
...
When `gh`, `pnpm`, build, test, or generator commands fail from sandbox blocks (credentials, network, IPC, file watching, nested `sandbox-exec`), retry unchanged with the narrowest host escalation before diagnosing auth or project failure. Require sandbox evidence; never bypass genuine test failures or the product sandbox under test.
...
- Never default to the full suite or repeat a passing check. CI owns exhaustive coverage and the platform matrix; rehearse all locally only by explicit request, for CI diagnosis, or for an irreducibly repository-wide change.
...
- ESM everywhere (`"type": "module"`). Use package names across packages and `.ts` in local relative imports. Config subprocesses run built `lib/` under plain Node; source regressions use their declared launcher ([testing policy](docs/testing.md#test-subprocess-launch-modes)). The `dsh` CLI source launch runs through tsx's ESM-only hook (`node --import tsx/esm`); modules it reaches must stay ESM (no CJS-only exports) ([source-launch contract](.agents/notes/implemented/architecture/2026-07-29-dsh-source-launch-tsx-esm.md)). Raw/Web `cordis.yml` bare plugins must appear in their resolver manifest's `dependencies`; `verify-cordis-config` enforces it.
...
- **Prefer maintained dependencies over hand-rolling** when they delete owned code and tests ([policy](.agents/notes/implemented/process/2026-07-26-dependencies-over-hand-rolling.md)).
...
Comments and docs state behavior, failure, timing, ownership, and safe-use facts — not control-flow narration, review history, or code restatement. Prefer exact terms (`response fields`, `JSON validation`) over `shape`/`boundary`/`contract`, which are reserved for real obligations; see [dsh-prose-standard](.agents/skills/dsh-prose-standard/SKILL.md). Wire checkable invariants into an executed top-level gate covering each changed acceptance path; use narrow, justified exceptions instead of global rule disables.

Docs accompany every code change: update affected README and JSDoc together. Routine bilingual work follows [docs/AGENTS.md](docs/AGENTS.md); only explicit user invocation may run `dsh-translate-docs`.

## Editing these instructions

`CLAUDE.md` symlinks `AGENTS.md` at root, `packages/`, and `examples/`; edit the real file. Keep each rule self-contained while linking high-level docs.
